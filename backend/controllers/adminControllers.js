...
export const getAllBanners = async (req, res) => {
  try {
    const banners = await Banner.find().sort({ order: 1, createdAt: -1 }).populate('createdBy', 'name email');
    res.json({
      success: true,
      banners
    });
...
export const createBanner = async (req, res) => {
  try {
    const { title, description, image, link, isActive, order, startDate, endDate } = req.body;
