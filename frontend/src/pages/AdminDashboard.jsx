...
// Enhanced Booking Card with Better Visual Hierarchy and Collapsible Details
const BookingCard = ({ booking, onStatusUpdate, isLoading, type }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 hover:shadow-md transition-all duration-200">
      ...
      {/* Payment Details Section - Always visible for admins */}
      ...
      <div className="mb-4">
        <button
          ...
          className="w-full px-4 py-2.5 bg-gradient-to-r from-blue-50 to-indigo-50 hover:from-blue-100 hover:to-indigo-100 border-2 border-blue-200 text-blue-700 rounded-lg font-semibold transition-all duration-200 flex items-center justify-center gap-2"
        >
          <svg 
            className={`w-5 h-5 transition-transform duration-200 ${showFullDetails ? 'rotate-180' : ''}`} 
            fill="none" 
            stroke="currentColor" 
            viewBox="0 0 24 24"
          >
          ...
          <span>{showFullDetails ? 'Hide' : 'View'} Full Details</span>
        </button>
      </div>

      {/* Additional Contact & Location Details - Collapsible */}
      {showFullDetails && (
